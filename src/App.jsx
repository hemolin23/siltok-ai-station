...
const uiScenes = [
  ...
  ['06','设置','管理主机、主题、桌宠、保存位置与版本','settings'],
  ['07','桌宠','不离开创作界面，随时查看模型与设备状态','pet'],
];
...
function ProductUISuite() {
  ...
  return <section className={styles.uiSuite} id="product-ui">
    ...
    <div className={styles.uiSuiteFrame}>
      <div className={styles.uiSuiteMeta}><span>{uiScenes[active][0]} / 07</span><div><h3>{uiScenes[active][1]}</h3><p>{uiScenes[active][2]}</p></div></div>
...
function StudioProductScreen({type}) {
  const [chatMode, setChatMode] = useState('视频生成');
  if(type==='host') return <StudioShell resource="显存 8%"><div className={styles.studioPage}><div className={styles.studioProfile}><i/>清风铃兰 <button>退出登录</button></div><div className={styles.studioTitle}><h3>主机</h3><button>＋ 添加主机</button></div><section className={styles.hostCard}><div className={styles.hostInfoStack}><h4>▣ Siltok 设备 <small>● 在线 · 正在使用</small></h4><section><b>机器信息</b><dl><dt>机型</dt><dd>桌面创作站</dd><dt>系统</dt><dd>Windows 11</dd><dt>主机名</dt><dd>Siltok-Station</dd></dl></section><section><b>软件信息</b><dl><dt>Dash</dt><dd>V1.0</dd><dt>ComfyUI</dt><dd>已连接</dd></dl></section><section className={styles.hostMembers}><b>成员 · 3</b><div><i/>清风铃兰 <small>管理员</small></div><div><i/>创作者 02 <small>在线</small></div><div><i/>创作者 03 <small>离线</small></div></section></div><div className={styles.hostMetricSide}><div className={styles.hostMetrics}><article className={styles.hostWideMetric}><span>存储</span><b>366 / 930 GB</b><i style={{'--meter':'39%'}}/></article><article className={styles.hostWideMetric}><span>网络</span><b>↓ 1.72　↑ 0.33 MB/s</b><i style={{'--meter':'18%'}}/></article>{[['CPU','17%'],['内存','78%'],['GPU','15%'],['显存','8%']].map(([label,value])=><article className={styles.hostGauge} style={{'--gauge':value}} key={label}><i/><span>{label}</span><b>{value}</b></article>)}</div><section className={styles.hostActivity}><div><b>设备活跃度</b><span>近 8 个月 · 128 次任务</span></div><div>{Array.from({length:72},(_,i)=><i key={i} data-level={(i*7+i%5)%4}/>)}</div><small>少　　　　　　　　　　　　　　　多</small></section></div></section><div className={styles.hostMini}><article><b>Siltok 设备</b><span>● 在线</span><button>使用</button></article><article><b>Siltok-1</b><span>● 在线</span><button>使用</button></article><article><b>＋</b><span>添加主机</span></article></div></div></StudioShell>;
  ...
  if(type==='inspiration') return <StudioShell><div className={styles.studioPage}><div className={styles.studioTitle}><div><h3>灵感广场</h3><p>发现可直接运行的开源工作流与创作模板</p></div></div><div className={styles.inspirationTabs}><b>灵感广场</b><span>管理我的 ComfyUI</span><button>全部</button><button>产品</button><button>人像</button><input value="" readOnly placeholder="搜索模板和能力"/></div><div className={styles.workflowCards}>{[['Ideogram 4','图片文字与排版',showreel[2].poster],['Krea 2 Turbo','文生图',showreel[0].poster],['Z-Image Base','轻量文生图',showreel[1].poster],['LTX 2.3 IC-LoRA HDR','图像生成视频',showreel[3].poster],['LTX 2.3 Lipdub','音画同步',showreel[0].poster],['LTX 2.3 Motion Track','运动控制',showreel[2].poster],['LTX 2.3 单阶段 T2V / I2V','快速生成视频',showreel[1].poster],['LTX 2.3 双阶段 T2V / I2V','高质量生成',showreel[3].poster],['LTX 2.3 Union Control','多控制联合',showreel[0].poster]].map(([name,copy,poster])=><article key={name}><img src={asset(poster)} alt=""/><span>WORKFLOW</span><b>{name}</b><small>{copy}</small></article>)}</div></div></StudioShell>;

  if(type==='chat') return <StudioShell conversations resource="内存 90%" queue="队列 1"><div className={styles.realChatStage}><div className={styles.chatModeTabs}>{['文字生成','图片生成','视频生成'].map(mode=><button key={mode} onClick={()=>setChatMode(mode)} className={chatMode===mode?styles.chatModeOn:''}>{mode}</button>)}</div><div className={styles.realResultMeta}>{chatMode==='文字生成'?'Gemma-4 · 本地语言模型':chatMode==='图片生成'?'SenseNova · 1:1 · 2K · 1 张':'minimax-h3-fl2va · 16:9 · 768P · 5 秒'}</div><div className={styles.realResult}>{chatMode==='视频生成'&&<video src={asset(showreel[2].src)} poster={asset(showreel[2].poster)} muted autoPlay loop playsInline/>}{chatMode==='图片生成'&&<img src={asset(showreel[3].poster)} alt="龙临峡谷生成结果"/>}{chatMode==='文字生成'&&<div className={styles.realTextReply}><span>GEMMA-4 / LOCAL</span><h3>把创作想法整理为可执行镜头</h3><p>画面从机械法老的金属面部开始，冷光沿着雕刻纹路缓慢流动；镜头向右侧环绕，逐渐露出内部机械结构，并保持主体身份与材质稳定。</p></div>}</div><div className={styles.realActions}><button>重新编辑</button><button>再次生成</button><button>复制</button><button>删除</button></div><div className={styles.realComposer}><p>{chatMode==='文字生成'?'输入问题、剧本或提示词…':chatMode==='图片生成'?'描述想生成的画面，也可以添加参考图。':'描述主体、动作、场景和镜头效果。'}</p><div><button className={styles.realActive}>{chatMode}</button><span>{chatMode==='文字生成'?'Gemma-4 语言模型':chatMode==='图片生成'?'SenseNova 图像模型':'minimax h3'}⌄</span>{chatMode==='图片生成'&&<><span>文字生成图片⌄</span><span>1:1 · 2K · 1张⌄</span></>}{chatMode==='视频生成'&&<><span>文字生成视频⌄</span><span>16:9 · 768P · 5秒 · 1条⌄</span></>}<i>↑</i></div></div></div></StudioShell>;

  if(type==='settings') return <StudioShell conversations resource="GPU 90%" queue="队列 3"><div className={styles.settingsScene}><div className={styles.settingsHint}><span>设置</span><b>账户、设备与偏好集中管理</b></div><aside className={styles.settingsPanel}><div className={styles.settingsUser}><i/>清风铃兰 <b>›</b></div><div><span>▣ 主机</span><small>设备 × 6</small><b>›</b></div><div><span>◌ 主题</span><small className={styles.themeChoice}><i>☾</i><i>☀</i><i>▣</i></small></div><div><span>♟ 显示桌宠</span><button className={styles.settingSwitch}><i/></button></div><div><span>⌂ 保存位置</span><small>…/Downloads/Siltok Dash</small></div><div><span>◷ 版本更新</span><small>2026091520　检查更新</small></div><button className={styles.logoutButton}>↪ 退出登录</button></aside><div className={styles.realComposer}><p>输入消息，与模型对话…</p><div><button>文字生成</button><span>Gemma-4 语言模型⌄</span><i>↑</i></div></div></div></StudioShell>;

  if(type==='pet') return <StudioShell conversations resource="内存 92%" queue="队列 3"><div className={styles.petScene}><div className={styles.petResult}><img src={asset(showreel[0].poster)} alt="生成结果"/><p>创作任务继续在后台运行，桌宠会同步当前模型与资源状态。</p></div><aside className={styles.petStatus}><header><b>Siltok 小鸟</b><span>运行中</span><i>×</i></header><dl><dt>当前模型</dt><dd>local/minimax-h3-fl2va</dd><dt>当前任务</dt><dd>1 个 ↑</dd><dt>GPU 占用</dt><dd>100%</dd><dt>显存占用</dt><dd>16%</dd></dl><div><button>打开 Dash ↗</button><button>表演一下</button></div></aside><div className={styles.petBird} aria-label="Siltok 桌宠"><i/><b>●</b></div><div className={styles.realComposer}><p>输入消息，与模型对话…</p><div><button>文字生成</button><span>Gemma-4 语言模型⌄</span><i>↑</i></div></div></div></StudioShell>;
